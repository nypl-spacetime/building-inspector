class ClusterSheetFlagsForTask < ActiveRecord::Migration
  def up
...
BEGIN
    ...
    EXECUTE format('CREATE TEMPORARY TABLE tmp AS SELECT F.id AS id, F.polygon_id AS polygon_id, F.latitude, F.longitude, F.session_id, F.flag_value, ST_GeometryFromText('''||Concat('POINT(''||','latitude','||'' ''||','longitude','||'')')||''') AS geom FROM flags F, polygons P WHERE F.polygon_id = P.id AND P.sheet_id = %L AND F.flag_type = %L AND F.latitude IS NOT NULL AND F.longitude IS NOT NULL ', sheet_id, task);
    ...
    RETURN QUERY EXECUTE 'SELECT dmn, id, polygon_id, session_id, flag_value, latitude, longitude FROM tmp ORDER BY dmn';
...
-- USAGE:
-- SELECT * FROM cluster_sheet_flags_for_task(2, 203, 'address') AS g(dmn integer, id integer, polygon_id integer, session_id varchar, flag_value text, latitude numeric, longitude numeric);
    SPROC
  end

  def down
    execute "DROP FUNCTION IF EXISTS cluster_sheet_flags_for_task(radius numeric, sheet_id integer, task varchar)"
  end
end
